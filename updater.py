...
class Retriever:
    ...
    def start(self):
        ...
        if self.codeforces:
            ...
            if self.get_gym and self.cf_password is None:
                self.cf_password = getpass(
                    'Password is needed for gym contests, please enter your password: ')
            if self.split_gym is None:
                self.split_gym = self.get_input(
                    'Separate regular and gym contests in different folders?: [y/n] ')
        ...
        if self.codeforces:
            if self.verbose:
                print('Downloading codeforces submissions for handle: '+self.cf_handle)
            self.gym_set = set()
            ...
            with requests.Session() as self.req:
                try:
                    ...
                    if self.get_gym and self.gym_set:
                        ...
                        if self.split_gym:
                            self.check_path(os.path.join(os.path.join(
                                'codeforces', self.cf_handle), 'gym'))
                        else:
                            self.check_path(os.path.join(
                                'codeforces', self.cf_handle))
                    if self.get_regular and self.contests_set:
                        if self.split_gym:
                            self.check_path(os.path.join(os.path.join(
                                'codeforces', self.cf_handle), 'normal'))
                        else:
                            self.check_path(os.path.join(
                                'codeforces', self.cf_handle))
                    self.data = self.req.get(
                        'http://codeforces.com/api/user.status?handle={}'.format(self.cf_handle)).json()
                    self.get_submissions()
                    self.set_downloaded('codeforces', self.cf_handle)
                    if self.errors and self.verbose:
                        print(
                            "Codeforces submissions for the following problems weren't downloaded:")
        ...
        if self.spoj:
            ...
            with requests.Session() as self.req:
                try:
                    ...
                    if self.errors and self.verbose:
                        print(
                            "SPOJ submissions for the following problems weren't downloaded:")
                        for error in set(self.errors):
                            print(error)
                        print('Run one more time to download them.')
                    if self.verbose:
    ...
    def spoj_login(self):
        data = {'next_raw': '/', 'autologin': 1,
                'login_user': self.spoj_handle, 'password': self.spoj_password}
    ...
    def get_info(self):
        for i, gym_status in enumerate(('false', 'true')):
            data = self.req.get(
                'http://codeforces.com/api/contest.list?gym={}'.format(gym_status)).json()
            if data['status'] != 'OK':
    ...
    def get_submissions(self):
        ...
        submissions = (Submission('codeforces', raw_data, raw_data['contestId'] in self.gym_set,
                                  self.cf_handle, self.split_gym is False) for raw_data in self.data['result'])
    ...
    def get_spoj_submissions(self):
        ...
        for row in rows:
            ...
            submission = Submission('spoj', {'language': lang, 'source': page.text,
                                             'problem': splitted[0], 'id': sub_id}, False, self.spoj_handle, False)
            if self.verbose:
                print('Downloading --> {}'.format(submission))
    ...
    def get_source_code(self, submission):
        ...
        page = self.req.get(self.get_url.format(contest_type=contest_type,
                                                contest_id=submission.get_contest(), submission_id=submission.get_id()))
        time.sleep(2)
        soup = bs(page.text, 'html.parser')
    ...
    def process_submission(self, submission):
        if self.folders:
            ...
            file_path = os.path.join(submission.get_directory(
