...
int main() {
    ...
    for (int z = 0; z < t; ++z)        /* code */
    {
        ...
        vl ansarr;
        if(n==1){
            cout<<"Case #"<<z+1<<": "<<(float)(k-1)/(float)k<<endl;
            continue;
        }
        for (int i = 1; i < n; ++i)
        {
            // out1(arr[i]);
            if((arr[i]-arr[i-1]-1)%2==0){
                ansarr.push_back((arr[i]-arr[i-1]-1)/2);
                ansarr.push_back((arr[i]-arr[i-1]-1)/2);
            }
            else{
                ansarr.push_back((arr[i]-arr[i-1])/2);
                ansarr.push_back((arr[i]-arr[i-1]-2)/2);
            }
        }
        ansarr.push_back(arr[0]-1);
        ansarr.push_back(k-arr[n-1]);
        vdsort(ansarr);

        
        double final=ansarr[0]+ansarr[1];
        cout<<"Case #"<<z+1<<": "<<(final/k)<<endl;
      // arrout(ansarr,ansarr.size());
    }
    return 0;
}
